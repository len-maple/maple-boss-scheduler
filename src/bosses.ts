export interface Boss {
  id: string
  name: string
  // public/bosses/ からの相対パス(先頭にスラッシュを付けない)。実ファイルは著作権の関係で
  // リポジトリに同梱していないため、docs/adding-boss-images.md を参照して各自で用意してください。
  // 実際のURLは BossImage コンポーネント側で import.meta.env.BASE_URL と結合する
  // (vite.config.ts の base 設定に対応するため)。
  image: string
}
